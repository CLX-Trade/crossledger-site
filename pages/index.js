...
// Public read-only RPC fallback when no wallet is connected
const READONLY_RPC = "https://cloudflare-eth.com";
...
export default function HomePage() {
  ...
  const [clxtPerUsdt, setClxtPerUsdt]             = useState(10n);
  ...
  const [contactStatus, setContactStatus]         = useState(null); // {kind, text}
  ...
  const amountWei = useMemo(() => {
    ...
    try { return ethers.parseUnits(String(usdtAmount), 6); } catch { return 0n; }
  ...
  const hasBalance   = amountValid && usdtBalance >= amountWei;
  const hasAllowance = amountValid && usdtAllowance >= amountWei;

  /* ====== HELPERS ====== */
  const setMsg = (kind, html) => setStatusMsg(html ? { kind, html } : null);
  const shortAddr = (a) => a ? a.slice(0, 6) + "…" + a.slice(-4) : "";
  const fmtUsdt = (n) => {
    const v = ethers.formatUnits(n, 6);
    return parseFloat(v).toLocaleString("en-US", { maximumFractionDigits: 2 }) + " USDT";
  };
  ...
  const loadPresaleInfo = useCallback(async () => {
    try {
      const provider = new JsonRpcProvider(READONLY_RPC);
      const presale = new ethers.Contract(PRESALE_CONTRACT_ADDRESS, PRESALE_ABI, provider);
      const [rate, active] = await Promise.all([
        presale.clxtPerUsdt().catch(() => 10n),
        presale.presaleActive().catch(() => true),
      ]);
      setClxtPerUsdt(rate);
      setPresaleActive(!!active);
      if (!active) {
        setMsg("warn", "<strong>Presale paused.</strong> The contract is currently not accepting purchases.");
      }
    } catch (e) {
      console.error("loadPresaleInfo failed", e);
    }
  }, []);
  ...
  const loadWalletData = useCallback(async (addr) => {
    ...
    try {
      ...
      const [bal, allow] = await Promise.all([
        usdt.balanceOf(addr),
        usdt.allowance(addr, PRESALE_CONTRACT_ADDRESS),
      ]);
      setUsdtBalance(bal);
      setUsdtAllowance(allow);
    } catch (e) {
  ...
  async function connectWallet({ silent = false } = {}) {
    ...
    try {
      ...
      const onChain = network.chainId === 1n;
      setWalletAddress(addr);
      setChainOk(onChain);
      if (!onChain) {
        setMsg("warn", "<strong>Wrong network.</strong> Please switch to Ethereum Mainnet.");
      } else {
        setMsg(null);
        await loadWalletData(addr);
      }
    } catch (e) {
      console.error("connectWallet failed", e);
      if (e.code === 4001) setMsg("warn", "Connection rejected. Click <strong>Connect Wallet</strong> when ready.");
      else setMsg("error", "Could not connect wallet: " + (e.shortMessage || e.message || "unknown error"));
    }
  }
  ...
  async function restoreExistingWallet() {
    ...
    try {
      ...
      if (accounts && accounts[0]) await connectWallet({ silent: true });
    } catch (e) { /* ignore */ }
  }
  ...
  async function handleApproveUSDT() {
    if (pending || !amountValid || !hasBalance) return;
    setPending(true);
    ...
    try {
      ...
      const tx = await usdt.approve(PRESALE_CONTRACT_ADDRESS, amountWei);
      setMsg("info", `Approval submitted: <a href="https://etherscan.io/tx/${tx.hash}" target="_blank" rel="noopener">${shortAddr(tx.hash)} ↗</a>`);
      await tx.wait();
      ...
      const m = e.code === "ACTION_REJECTED" ? "Approval cancelled." : (e.shortMessage || e.message || "Unknown error");
      setMsg("error", "<strong>Approval failed.</strong> " + m);
    } finally {
      setPending(false);
    }
  }
  ...
  async function handleBuy() {
    if (pending || !amountValid || !hasBalance || !hasAllowance) return;
    setPending(true);
    ...
    try {
      ...
      setMsg("info", `Purchase submitted: <a href="https://etherscan.io/tx/${tx.hash}" target="_blank" rel="noopener">${shortAddr(tx.hash)} ↗</a>`);
      const receipt = await tx.wait();
      const v = parseFloat(usdtAmount);
      const clxtReceived = (v * Number(clxtPerUsdt)).toLocaleString("en-US");
      setMsg("info", `<strong>Purchase confirmed.</strong> ${clxtReceived} CLXT sent to your wallet. <a href="https://etherscan.io/tx/${receipt.hash}" target="_blank" rel="noopener">View transaction ↗</a>`);
      setUsdtAmount("");
      await loadWalletData(walletAddress);
    } catch (e) {
      console.error("buy failed", e);
      let m = e.shortMessage || e.message || "Unknown error";
      if (e.code === "ACTION_REJECTED") m = "Purchase cancelled.";
  ...
  /* ====== EFFECTS ====== */
  useEffect(() => { loadPresaleInfo(); }, [loadPresaleInfo]);
  useEffect(() => { restoreExistingWallet(); }, []);
  useEffect(() => {
    if (typeof window === "undefined" || !window.ethereum) return;
  ...
  const buttonState = useMemo(() => {
    if (typeof window === "undefined" || !window.ethereum) {
      return { left: "Install MetaMask", leftAction: () => window.open("https://metamask.io/download/", "_blank"), leftDisabled: false, right: "Buy CLXT", rightDisabled: true };
    }
    if (!walletAddress) {
    ...
    if (!hasAllowance) {
      return { left: pending ? "Approving…" : "Approve USDT", leftAction: handleApproveUSDT, leftDisabled: pending, right: "Buy CLXT", rightDisabled: true };
    }
    return { left: "✓ USDT Approved", leftDisabled: true, right: pending ? "Buying…" : "Buy CLXT", rightAction: handleBuy, rightDisabled: pending };
  }, [walletAddress, chainOk, presaleActive, tosChecked, amountValid, hasBalance, hasAllowance, pending]);
    ...
    <>
        ...
        <div className="wrap">
          <div className="purchase">
            <div>
              ...
              <div className="steps">
                <div className="step"><div className="n">01</div><h4>Connect wallet</h4><p>MetaMask on desktop or mobile. Confirm Ethereum mainnet.</p></div>
            ...
            <div id="presale" className="widget">
              ...
              <div className="row"><span className="k">Wallet Status</span><span className={`v ${walletAddress ? (chainOk ? "success" : "warn") : "muted"}`}>{walletAddress ? (chainOk ? shortAddr(walletAddress) : shortAddr(walletAddress) + " · Wrong network") : "Not connected"}</span></div>
              <div className="row"><span className="k">USDT Balance</span><span className="v">{walletAddress && chainOk ? fmtUsdt(usdtBalance) : "—"}</span></div>
              <div className="row"><span className="k">Approved Allowance</span><span className="v">{walletAddress && chainOk ? fmtUsdt(usdtAllowance) : "—"}</span></div>
              ...
              <div className="actions">
                ...
                >
                  {pending && (buttonState.left.includes("Approving") || buttonState.left.includes("USDT Approved")) ? <span className="spinner" /> : null}
                  {buttonState.left}
                </button>
                <button
                  type="button"
                ...
                >
                  {pending && buttonState.right.includes("Buying") ? <span className="spinner" /> : null}
                  {buttonState.right}
                </button>
              </div>
      ...
      <section className="cream tight">
        <div className="wrap">
          ...
          <div className="trans-grid">
            <a className="trans-cell" href={`https://etherscan.io/address/${PRESALE_CONTRACT_ADDRESS}`}><div className="k">PRESALE CONTRACT</div><div className="v">0xABCA…2B35 ↗</div></a>
      ...
      <section id="faq" className="cream-deep">
        <div className="wrap">
          ...
          <div className="faq-list">
            ...
            <details className="faq"><summary>What currency do buyers use, and on what network?</summary><div className="answer"><p>Purchases are made in USDT (Tether) on the Ethereum mainnet. CLXT is an ERC-20 token. You will need a compatible Ethereum wallet such as MetaMask, sufficient USDT for the purchase amount, and a small amount of ETH for gas.</p></div></details>
      ...
      <style jsx global>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .ps-block.problem .ps-list li::before { content: '✗'; background: rgba(181,74,64,0.12); color: var(--danger); }
        .ps-block.solution .ps-list li::before { content: '✓'; background: var(--gold-soft); color: var(--gold-deep); }
        ...
        .spinner { display: inline-block; width: 12px; height: 12px; border: 2px solid currentColor; border-right-color: transparent; border-radius: 50%; animation: spin 0.8s linear infinite; vertical-align: -2px; margin-right: 6px; }
