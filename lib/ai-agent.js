 ...
 */
async function runImprovementAgent(pulseData) {
...
async function applyLivePatches(improvements, pulseData) {
  ...
  // --- Patch 2: Update market context headline in index.js ---
  if (ethPrice || btcPrice || sentiment) {
    try {
      const indexPath = path.resolve('pages/index.js');
      let indexContent = readFileSync(indexPath, 'utf8');

      const ethStr = ethPrice ? `ETH at US${Math.round(ethPrice).toLocaleString()}` : null;
      const btcStr = btcPrice ? `BTC at US${Math.round(btcPrice).toLocaleString()}` : null;
      const sentimentCap = sentiment.charAt(0).toUpperCase() + sentiment.slice(1);
      const marketParts = [btcStr, ethStr].filter(Boolean);
      const marketLine = marketParts.length > 0
        ? `${marketParts.join(' · ')} · Sentiment: ${sentimentCap}`
        : `Sentiment: ${sentimentCap}`;

      // Update the market context h2 — matches the existing static copy
      const contextH2Regex = /(<h2 className="h-section"[^>]*>)While crypto markets consolidate[^<]*(<\/h2>)/;
      if (contextH2Regex.test(indexContent)) {
        const sentimentMsg = sentiment === 'bullish'
          ? 'Momentum builds in digital asset markets'
          : sentiment === 'bearish'
          ? 'As markets rebalance, real-world infrastructure holds its ground'
          : 'While crypto markets consolidate, institutional capital flows into real-world asset infrastructure';
        indexContent = indexContent.replace(
          contextH2Regex,
          `$1${sentimentMsg}. CrossLedger bridges traditional commodity trade with blockchain efficiency.</h2>`
        );
        writeFileSync(indexPath, indexContent, 'utf8');
        console.log('[ai-agent] Patched: index.js market context updated —', marketLine);
      } else {
        console.log('[ai-agent] Market context pattern not found in index.js — skipping');
      }
    } catch (err) {
...
function formatPRDescription(improvements, pulseData) {
    const market = pulseData?.market;
...
module.exports = { runImprovementAgent, formatPRDescription };
